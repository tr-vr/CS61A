...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    index, total, calc_score = 0, 0, score**3
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            curr_roll = take_turn(dice_num, score1, dice)
            score0 += curr_roll
            if feral_hogs:
                if abs(dice_num - prev_roll0) == 2:
                    score0 += 3
                prev_roll0 = curr_roll
                
        elif who == 1:
            dice_num = strategy1(score1, score0)
            curr_roll = take_turn(dice_num, score0, dice)
            score1 += curr_roll
            if feral_hogs:
                if abs(dice_num - prev_roll1) == 2:
                    score1 += 3
                prev_roll1 = curr_roll
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        if who == 0 and score0 - ps > ph:
            prev_high = score0 - ps
            ps = score0
            print(prev_high + "point(s)! That's the biggest gain yet for Player 0")
        elif who == 1 and score1 - ps > ph:
            prev_high = score1 - ps
            ps = score1
            print(prev_high + "point(s)! That's the biggest gain yet for Player 1")
    return say
    # END PROBLEM 7
